...
// find . -name "*.dot" | while read file; do dot -Tpng -o${file/.dot/.png} $file; done
...
abin insert(int x, abin a)
{
  ...
  if(ant == NULL) a = new; // prev. vazia (se ant é NULL então não entrou no ciclo)
...
void test_remove_leaf(void)
{
  ...
  printf("Teste 1: remover folha (20)\n");
...
void test_remove_one_child(void)
{
  ...
  printf("Teste 2: remover nó com um filho (40)\n");
  printf("  antes: size = %d, depth = %d\n", size(a), depth(a));
...
void test_remove_two_children(void)
{
  ...
  printf("Teste 3: remover nó com dois filhos (70)\n");
  printf("  antes: size = %d, depth = %d\n", size(a), depth(a));
...
void test_remove_root(void)
{
  ...
  printf("Teste 4: remover a raiz (50)\n");
  printf("  antes: size = %d, depth = %d\n", size(a), depth(a));
...
void test_remove_missing(void)
{
  ...
  printf("Teste 5: remover valor inexistente (999)\n");
  printf("  antes: size = %d, depth = %d\n", size(a), depth(a));
